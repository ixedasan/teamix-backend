...
export class TaskLabelsResolver {
	public constructor(private readonly taskLabelsService: TaskLabelsService) {}
	...
	@Mutation(() => Boolean, { name: 'createTaskLabel' })
	...
	@Mutation(() => Boolean, { name: 'addLabelToTask' })
	public async addLabelToTask(
		@Args('taskId') taskId: string,
		@Args('labelId') labelId: string
	) {
		return this.taskLabelsService.addLabelToTask(taskId, labelId)
	}

	@UseGuards(GqlAuthGuard, ProjectGuard)
	@RolesAccess(Role.ADMIN, Role.MEMBER)
	@Mutation(() => Boolean, { name: 'removeLabelFromTask' })
	public async removeLabelFromTask(
		@Args('taskId') taskId: string,
		@Args('labelId') labelId: string
	) {
		return this.taskLabelsService.removeLabelFromTask(taskId, labelId)
	}
